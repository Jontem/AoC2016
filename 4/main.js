...
for (const room of rooms) {
    const [, roomName, sectorId, checksum] = regex.exec(room);

    const sortedCountedCharArray = getSortedCountedCharArray(roomName);

    if (isValidRoom(sortedCountedCharArray, checksum)) {
        validRooms.push({
...
for (const validRoom of validRooms) {
    ...
    for (const encryptedChar of validRoom.roomName) {
        if (encryptedChar === "-") {
            decryptedRoomName += " ";
        } else {
            decryptedRoomName += chars[(chars.indexOf(encryptedChar) + validRoom.sectorId) % 26];
        }
    }

    console.log(`${decryptedRoomName} ${validRoom.sectorId}`);
}

function getSortedCountedCharArray(roomName) {
    const charObj = {};
    ...
    return Object.keys(charObj)
        .reduce((soFar, key) => {
            return soFar.concat({char: key, count: charObj[key]});
        }, [])
        .sort((a, b) => {
            if (b.count === a.count) {
                if (a.char > b.char) {
...
function isValidRoom(sortedCountedCharArray, checksum) {
    for (const checksumChar of checksum) {
        const match = sortedCountedCharArray.find((obj) => obj.char === checksumChar);
