...
/*
 ...
 "dkszjaqhbkyxwvwj[wykmzcvppharrax]pvrqzqhabqtuhrj[qfxnormttaxsldn]kxjwkrjwvbiutgnfnw[mbspfflblosguqzt]nifozavfsfkngds",
 "abba[wykmzcvppharrax]pvrqzqhabqtuhrj[acca]kxjwkrjwvbiutgnfnw[mbspfflblosguqzt]nifozavfsfkngds"
 ];
 */
...
for (const ip of ips) {
    ...
    const abas = getAbas(superNets);
    for (const aba of abas) {
        const [a, b] = aba;
        const bab = b + a + b;

        if (findInArray(hyperNets, bab)) {
            if (supportsSSL.indexOf(ip) < 0) {
                supportsSSL.push(ip);
            }
        }
    }
...
console.log(`Supports SSL: ${supportsSSL.length}`);

function findInArray(arr, bab) {
    for (const str of arr) {
        if (str.indexOf(bab) > -1) {
            return true;
        }
    }
...
function getSuperNets(ip) {
    ...
    const superNets = [];
    while (ipString) {
        const firstIndex = ipString.indexOf("[");
        const secondIndex = ipString.indexOf("]");

        if (firstIndex > -1) {
            superNets.push(ipString.substr(0, firstIndex));
            ipString = ipString.substr(secondIndex + 1);
        } else {
            superNets.push(ipString);
            break
        }
    }
...
function getHyperNets(ip) {
    ...
    while (ipString) {
        ...
        if (firstIndex > -1) {
            hyperNets.push(ipString.substr(firstIndex + 1, secondIndex - firstIndex - 1));
            ipString = ipString.substr(secondIndex + 1);
        } else {
            break;
        }
...
function getAbas(superNets) {
    ...
    for(const superNet of superNets) {
        for (let i = 0; i < superNet.length; i++) {
            ...
            const possibleAba = superNet.substr(i, 3);

            if (possibleAba[0] === possibleAba[2] && possibleAba[0] !== possibleAba[1] && abas.indexOf(possibleAba) < 0) {
...
function getBabs(abas, hyperNets) {
    const babs = [];
    for (const aba of abas) {
        const [a, b] = aba;
        const bab = b + a + b;

        for(const str of hyperNets) {
            if(str.indexOf(bab) > -1 && babs.indexOf(bab) < 0) {
                babs.push(bab);
            }
        }
    }
